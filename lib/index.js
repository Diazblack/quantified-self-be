...
app.post('/api/v1/calendar', (request, response) => {
  const dates = request.body
  database('dates').insert({
    date_str: dates.date,
    goal: dates.goal,
    consumed: dates.consumed,
    remaining: dates.remaining,
    meals: JSON.stringify(dates.meals)
  })
    .then(date => {
...
app.get('/api/v1/calendar', (request, response) => {
