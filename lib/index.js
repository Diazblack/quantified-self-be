...
app.delete('/api/v1/meals/:meal_id/foods/:food_id', (request, response) => {
  database('mealFoods')
  .select('foods.name AS food', 'meals.name AS meal', 'mealFoods.id AS id')
  ...
  .first()
    .then(data =>{
      database('mealFoods').where('id', data.id).del()
      return data;
    })
      .then(dataMessage => {
        response.status(204).json({
          'message': `Successfully removed ${dataMessage.food} from ${dataMessage.meal}`
        })
      })
      .catch((error) =>{
        response.status(404).json({error});
