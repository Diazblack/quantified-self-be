...
// helper functions
function foodObject(meal) {
  return { "id": meal.id, "name": meal.name, "calories": meal.calories }
}
...
function mealNotation(meals) {
  ...
  for (var i = 0; i =< meals.length; i++ ) {
    ...
    if(meal.mealId === 1){
      brea.foods.push(foodObject(meal));
    }
    else if (meal.mealId === 2) {
      lunch.foods.push(foodObject(meal));
    }
    else if (meal.mealId === 3 ) {
      dinner.foods.push(foodObject(meal));
    }
    else {
      snacks.foods.push(foodObject(meal));
    }
  }
